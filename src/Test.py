...
def email_check():
    for i  in range(0, len(invalid_emails)):
        login_script(invalid_emails[i], registered_passwords[0])
        email_error = driver.find_element_by_id("emailError")
        assertCheck("Check invalid email test %d:" % (i+1), email_error.get_attribute('innerHTML'), "Please enter a valid e-mail.")

    for i  in range(0, len(unregistered_emails)):
        login_script(unregistered_emails[i], unregistered_passwords[0])
        login_error = driver.find_element_by_id("loginError")
        assertCheck("Check unregistered email test %d:" % (i+1), login_error.get_attribute('innerHTML'), "There is no such account. Please try signing up.")
    ...
    for i  in range(0, len(registered_emails)):
        ...
        assertCheck("Check registered email test %d:" % (i+1), welcome.get_attribute('innerHTML'), "Login Successful. Welcome " + registered_emails[i])
